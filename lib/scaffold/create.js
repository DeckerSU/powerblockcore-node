...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'powerblockcore-lib': 'https://github.com/DeckerSU/powerblockcore-lib', // '^' + bitcore.version,
    'powerblockcore-node': 'https://github.com/DeckerSU/powerblockcore-node' // version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/powerblockcore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the powerblockcore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the powerblockcore-node directory and configuration
