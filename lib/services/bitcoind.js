...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of powerblockcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of powerblockcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of powerblockcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to bitcoind from powerblockcore. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that bitcoind is undergoing a reindex. ' +
             ...
             'of powerblockcore-node services will start.');
    node._reindex = true;
  }
};
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/powerblockcoind.pid';
...
Bitcoin.prototype.getInfo = function(callback) {
  ...
  self.client.getBlockchainInfo(function(err, response) {
      ...
      self.client.getNetworkInfo(function(err, response) {
        ...
        info.protocolVersion = response.result.protocolversion;
        info.version = response.result.version;
        info.timeOffset = response.result.timeoffset;
        info.connections = response.result.connections,
