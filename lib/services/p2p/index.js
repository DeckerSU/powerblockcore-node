...
P2P.prototype._initP2P = function() {
  ...
  this.messages = new p2p.Messages({ 
    ...
    Block: Bcoin.block, 
    BlockHeader: Bcoin.headers,
    Transaction: BcoinTx,
    MerkleBlock: Bcoin.merkleblock,
...
P2P.prototype._onPeerInventory = function(peer, message) {
  ...
  message.inventory.forEach(function(inv) {
    ...
    if (!self._inv.get(inv.hash)) {
      ...
      // Segwit: set witness inv type
      if (inv.type == p2p.Inventory.TYPE.TX || inv.type == p2p.Inventory.TYPE.BLOCK) {
        inv.type = inv.type | p2p.Inventory.WITNESS_FLAG
      }
...
P2P.prototype._startBcoin = function(callback) {
  ...
  if (['livenet', 'live', 'main', 'mainnet'].indexOf(this.node.network) !== -1) {
    network = 'main';
    port = this._configPeers[0].port || 9333;
  } else if (this.node.network !== 'regtest') {
    network = 'testnet';
    port = this._configPeers[0].port || 19335;
  } else {
    network = this.node.network;
    port = this._configPeers[0].port || 49444;
  }
