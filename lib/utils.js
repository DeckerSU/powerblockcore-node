...
var Address = require('./lcoin-ext/address-ext');
...
utils.convertAddressArrayToMostCommonFormat = function(addrStrs) {
  if (_.isString(addrStrs)) {
    return utils.convertAddressToMostCommonFormat(addrStrs);
  }

  var results = [];
  _.forEach(addrStrs, function (addrStr) {
    results.push(utils.convertAddressToMostCommonFormat(addrStr));
  });

  return results;
}

utils.convertAddressToMostCommonFormat = function(addrStr) {
  assert(_.isString(addrStr));

  var address = Address.fromString(addrStr);
  address.convertFromSCRIPTHASH2ToSCRIPTHASH();
